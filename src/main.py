...
def make_question(user_input: str, session_id: str) -> str:
    try:
        logger.info("Agent invoked: router -> input = %s", user_input)
        ...
        if specialist := SPECIALISTS.get(agent_name):
            ...
            return invoke_specialist(specialist, router_response, session_id)

        if consultant := CONSULTANTS.get(agent_name):
            logger.info(
                "Consultant invoked: %s -> input = %s", agent_name, router_response
            )
            return invoke_consultant(consultant, router_response, session_id)
